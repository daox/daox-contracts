...
library DAOProxy {
    function delegatedInitState(address stateModule, uint _minVote, address _usersAddress, address _tokenAddress, address _votingFactory, address _serviceContract) {
        require(stateModule.delegatecall(bytes4(keccak256("initState(uint256,address,address,address,address)")), _minVote, _usersAddress, _tokenAddress, _votingFactory, _serviceContract));
    }
